            ...
            try {
                ...
                if (portForwardingL != null && portForwardingL.length > 0) {
                    for (int i = 0; i < portForwardingL.length; i++) {
                        String[] portForwarding = portForwardingL[i].split(":");
                        if (portForwarding != null && portForwarding.length == 3) {
                            if (portForwarding[1].equals(ssh.getRHost()) && portForwarding[2].equals(ssh.getRPort())) {
                                ssh.setLocalPort(portForwarding[0]);
                                return session;
                            }
                        }
                    }
