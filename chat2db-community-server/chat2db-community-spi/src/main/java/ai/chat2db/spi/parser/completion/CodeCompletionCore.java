...
public class CodeCompletionCore {
    ...
    private boolean translateToRuleIndex(int i, List<RuleWithStartToken> ruleWithStartTokenList) {
        ...
        if (preferredRules.contains(ruleIndex)) {
            ...
            int stopTokenIndex = ruleWithStartToken.stopTokenIndex;
            ...
            for (Map.Entry<Integer, CandidateRule> entry : candidates.rules.entrySet()) {
                ...
                if (samePath) {
                    if (stopTokenIndex > entry.getValue().stopTokenIndex
                            || (stopTokenIndex == entry.getValue().stopTokenIndex
                            && startTokenIndex > entry.getValue().startTokenIndex)) {
                        entry.getValue().startTokenIndex = startTokenIndex;
                        entry.getValue().stopTokenIndex = stopTokenIndex;
                    }
                    addNew = false;
                    break;
                }
            ...
            if (addNew) {
                candidates.rules.put(ruleIndex, new CandidateRule(startTokenIndex, stopTokenIndex, path));
                if (showDebugOutput) {
    ...
    private Set<Integer> processRule(RuleStartState startState, int tokenListIndex,
        ...
        RuleWithStartToken currentRule = new RuleWithStartToken(startTokenIndex, startTokenIndex,
                startState.ruleIndex);
        callStack.add(currentRule);
            ...
            if (preferredRules.contains(startState.ruleIndex)) {
                ...
                for (FollowSetWithPath set : followSets.sets) {
                    ...
                    List<RuleWithStartToken> followSetPath = set.path.stream()
                            .map(ruleIndex -> new RuleWithStartToken(startTokenIndex, startTokenIndex, ruleIndex))
                            .collect(Collectors.toList());
                ...
                switch (transition.getSerializationType()) {
                    case Transition.RULE: {
                        ...
                        for (int position : endStatus) {
                            currentRule.stopTokenIndex = Math.max(currentRule.stopTokenIndex,
                                    previousTokenIndex(position));
                        ...
                        if (set != null && set.size() != 0) {
                            ...
                            if (atCaret) {
                                ...
                                if (set.contains(currentSymbol)) {
                                    ...
                                    currentRule.stopTokenIndex = Math.max(currentRule.stopTokenIndex,
                                            tokens.get(currentEntry.tokenListIndex).getTokenIndex());
    ...
    private int previousTokenIndex(int tokenListIndex) {
        int previous = Math.min(tokenListIndex - 1, tokens.size() - 1);
        if (previous < 0) {
            return 0;
        }
        return tokens.get(previous).getTokenIndex();
    }
    ...
    public static class CandidateRule {
        ...
        public CandidateRule(int startTokenIndex, int stopTokenIndex, List<Integer> ruleList) {
    ...
    public static class RuleWithStartToken {
        ...
        public RuleWithStartToken(int startTokenIndex, int stopTokenIndex, int ruleIndex) {
            this.startTokenIndex = startTokenIndex;
            this.stopTokenIndex = stopTokenIndex;
