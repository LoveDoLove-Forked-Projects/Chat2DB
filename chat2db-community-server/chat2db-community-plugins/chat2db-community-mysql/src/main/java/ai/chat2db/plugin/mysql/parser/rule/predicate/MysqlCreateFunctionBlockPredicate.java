...
public class MysqlCreateFunctionBlockPredicate extends MysqlRulePredicate {


    public MysqlCreateFunctionBlockPredicate(int lookAheadTokens) {
        ...
        while (valuableTokenFound < lookaheadTokens) {
            ...
            if (TokenUtil.hasValuableText(token)) {
                if (tokenType == MySqlLexer.BEGIN) {
                    return true;
                } else if (tokenType == MySqlLexer.RETURN || tokenType == MySqlLexer.SEMI) {
                    return false;
                }
