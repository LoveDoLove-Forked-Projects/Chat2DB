...
export const pieDataTreating = (props: DataTreatingProps) => {
  ...
  const { valueField, colorField, angleField, orderByType, orderByRule } = chartSchema;
  ...
  const _xField = xField || angleField || 'name';
  const _yField = yField || valueField || colorField  || 'value';
