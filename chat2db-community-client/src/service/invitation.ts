...
const invitationService =
  __RUNTIME_ENV__ === 'community'
    ...
    : (() => {
        const createInvitationCode = createRequest<{ code: string }, boolean>('/api/invitation/create_invitation_code', {
          method: 'post',
        });

        const getMyInvitationCode = createRequest<void, string>('/api/invitation/get_my_invitation_code');

        const getInvitationCodeExit = createRequest<{ code: string }, boolean>('/api/invitation/get_invitation_code_exit');

        const getInvitationOrderItem = createRequest<void, InvitationOrderVO>('/api/invitation/get_invitation_order_item');
